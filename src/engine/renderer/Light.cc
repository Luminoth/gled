...
Logger& Light::logger(Logger::instance("gled.engine.renderer.Light"));
...
bool Light::load_colordef(const std::string& name)
{
    ...
    LOG_INFO("Loading colordef '" << filename << "'...\n");
...
void PositionalLight::destroy(PositionalLight* const light, MemoryAllocator* const allocator)
{
    light->~PositionalLight();
    operator delete(light, 16, *allocator);
}
...
void DirectionalLight::destroy(DirectionalLight* const light, MemoryAllocator* const allocator)
{
...
void SpotLight::destroy(SpotLight* const light, MemoryAllocator* const allocator)
{
    light->~SpotLight();
    operator delete(light, 16, *allocator);
}
