...
Logger& Font::logger(Logger::instance("gled.engine.renderer.Font"));
FT_Library Font::freetype = NULL;
GLuint Font::texture = 0;
GLuint Font::vbo[VBOCount] = { 0 };

bool Font::init()
{
    if(FT_Init_FreeType(&freetype)) {
        LOG_CRITICAL("Could not init freetype library!\n");
        return false;
    }
...
bool Font::load(const std::string& name, size_t height)
{
    const boost::filesystem::path& path(Engine::instance().resource_manager().font_path(name));
    if(FT_New_Face(freetype, path.string().c_str(), 0, &_face)) {
        LOG_ERROR("Could not open font '" << path << "'!\n");
        return false;
    }
...
void Font::render(const std::string& text, const Position& position, const Vector2& scale, bool center) const
{
    glEnable(GL_BLEND);
...
/*void Font::render(const std::string& text, const Camera& camera, const Position& position, const Vector2& scale, bool center) const
{
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
std::string Font::str() const
{
    std::stringstream ss;
    ss << "Font(color:" << _color.str() << ")";
    return ss.str();
}

void Font::calculate_width_height(const std::string& text, float& width, float& height) const
{
