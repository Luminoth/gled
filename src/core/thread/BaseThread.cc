...
BaseThread::BaseThread(ThreadPool* pool)
    : _pool(pool), _quit(false), _thread(NULL), _own_thread(false)
{
}

BaseThread::BaseThread(const std::string& name)
    : _pool(NULL), _name(name), _quit(false), _thread(NULL), _own_thread(false)
{
}
...
std::string BaseThread::str() const
{
    ...
    ss << "Name: " << name() << "\n"
        << "Has thread: " << to_string(_thread != NULL) << "\n"
        << "Owns thread: " << to_string(_own_thread) << "\n"
        << "Has pool: " << to_string(_pool != NULL) << "\n"
    ...
    while(!should_quit() && !boost::this_thread::interruption_requested()) {
        try {
            if(pool()) {
                ...
                if(guard.owns_lock()) {
                    if(pool()->has_work()) {
                        boost::shared_ptr<BaseJob> job(pool()->pop_work());
                        job->process_work();
                    }
                }
            } else {
