...
#define LOG_DEBUG(e) do { \
    ...
    logger << energonsoftware::Logger::LogLevelDebug \
        << boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time()) << " " \
        << "[" << boost::this_thread::get_id() << "] " \
        << logger.category() << " " \
        << energonsoftware::Logger::level(energonsoftware::Logger::LogLevelDebug) << ": " \
        << e; \
} while(false)

#define LOG_INFO(e) do { \
    boost::lock_guard<boost::recursive_mutex> guard(energonsoftware::Logger::logger_mutex); \
    logger << energonsoftware::Logger::LogLevelInfo \
        << boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time()) << " " \
        << "[" << boost::this_thread::get_id() << "] " \
        << logger.category() << " " \
        << energonsoftware::Logger::level(energonsoftware::Logger::LogLevelInfo) << ": " \
        << e; \
} while(false)

#define LOG_WARNING(e) do { \
    boost::lock_guard<boost::recursive_mutex> guard(energonsoftware::Logger::logger_mutex); \
    logger << energonsoftware::Logger::LogLevelWarning \
        << boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time()) << " " \
        << "[" << boost::this_thread::get_id() << "] " \
        << logger.category() << " " \
        << energonsoftware::Logger::level(energonsoftware::Logger::LogLevelWarning) << ": " \
        << e; \
} while(false)

#define LOG_ERROR(e) do { \
    boost::lock_guard<boost::recursive_mutex> guard(energonsoftware::Logger::logger_mutex); \
    logger << energonsoftware::Logger::LogLevelError \
        << boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time()) << " " \
        << "[" << boost::this_thread::get_id() << "] " \
        << logger.category() << " " \
        << energonsoftware::Logger::level(energonsoftware::Logger::LogLevelError) << ": " \
        << e; \
} while(false)

#define LOG_CRITICAL(e) do { \
    boost::lock_guard<boost::recursive_mutex> guard(energonsoftware::Logger::logger_mutex); \
    logger <<energonsoftware:: Logger::LogLevelCritical \
        << boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time()) << " " \
        << "[" << boost::this_thread::get_id() << "] " \
        << logger.category() << " " \
        << energonsoftware::Logger::level(energonsoftware::Logger::LogLevelCritical) << ": " \
        << e; \
} while(false)
...
public:
    ...
    template<typename T> friend Logger& operator<<(Logger& lhs, const T& rhs)
    {
        ...
        if(lhs.level() >= _logger_level) {
            ...
            BOOST_FOREACH(std::ostream* callback, _callbacks) {
                (*callback) << rhs;
            }
        }
        return lhs;
